...
export default async function ListaParceiros({
  ...
  const [nAtivos, nInativos, nBanidos, parceiros] = await Promise.all([
    prisma.parceiro.count({ where: { organizacaoId: orgId, status: "ATIVO" } }),
    prisma.parceiro.count({ where: { organizacaoId: orgId, status: "INATIVO" } }),
    prisma.parceiro.count({ where: { organizacaoId: orgId, status: "BANIDO" } }),
        ...
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
          {kpis.map((k) => (
            ...
            >
              ...
              <div style={{ fontSize: 24, fontWeight: 800, marginTop: 2, color: k.cor }}>
                {k.valor}
              </div>
            </div>
          ))}
                ...
                <tr key={p.id} style={{ borderTop: "1px solid #F0F0EF" }}>
                  <td style={tdEstilo}>
                    <a
                      ...
                      style={{ color: "#121111", textDecoration: "none", fontWeight: 600 }}
                    >
                      {p.nome}
                    </a>
